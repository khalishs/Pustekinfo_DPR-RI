{{-- resources/views/pdf/tiket-layanan.blade.php
     Template PDF tiket pengajuan layanan — dikirim sebagai lampiran email ke
     pemohon setelah submit form Ajukan Layanan. Dirender via dompdf, jadi
     CSS-nya sengaja dibuat sederhana (dompdf tidak mendukung flexbox/grid). --}}
<!DOCTYPE html>
<html lang="id">
<head>
<meta charset="UTF-8">
...
<body>
  ...
  <div class="header">
    <table>
      <tr>
        <td style="width:50%;"><img src="{{ $logoBase64 }}" alt="Pustekinfo"></td>
        <td style="width:50%;">
    ...
    <table>
      <tr>
        <td>
          ...
          <div class="kode-value">{{ $serviceRequest->kode }}</div>
        </td>
        <td style="width:140px;text-align:right;">
          <span class="status-pill">{{ \App\Models\ServiceRequest::STATUSES[$serviceRequest->status] ?? 'Diajukan' }}</span>
        </td>
      </tr>
    </table>
  </div>
  ...
  <table class="data">
    <tr>
      ...
      <td class="value">{{ $serviceRequest->nama }}</td>
    </tr>
    <tr>
      <td class="label">Email</td>
      <td class="value">{{ $serviceRequest->email }}</td>
    </tr>
    <tr>
      <td class="label">No. WhatsApp/Telepon</td>
      <td class="value">+{{ $serviceRequest->no_tlpn }}</td>
    </tr>
    @if(!empty($serviceRequest->instansi))
    <tr>
      <td class="label">Unit Kerja/Instansi</td>
      <td class="value">{{ $serviceRequest->instansi }}</td>
    </tr>
    @endif
    <tr>
      <td class="label">Jenis Layanan</td>
      <td class="value">{{ $serviceRequest->jenis_layanan }}</td>
    </tr>
    <tr>
      <td class="label">Tanggal Pengajuan</td>
      <td class="value">{{ $serviceRequest->created_at->locale('id')->translatedFormat('d F Y, H:i') }} WIB</td>
    </tr>
  </table>

  <div class="section-title">Detail Kebutuhan</div>
  <div class="pesan-box">{{ $serviceRequest->pesan }}</div>
  ...
  <div class="footer">
    ...
    Dokumen ini dibuat otomatis oleh sistem pada {{ now()->locale('id')->translatedFormat('d F Y, H:i') }} WIB dan tidak memerlukan tanda tangan.
  </div>
