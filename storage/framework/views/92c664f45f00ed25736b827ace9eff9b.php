...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .sukses-email{
    ...
    background:rgba(31,157,124,.1);color:#1f9d7c;font-size:12.5px;font-weight:700;
  }
  .sukses-email svg{width:15px;height:15px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;flex-shrink:0;}
  ...
  .ajukan-success-overlay{
    position:fixed;inset:0;z-index:10001;
    background:rgba(6,18,26,.55);backdrop-filter:blur(6px);-webkit-backdrop-filter:blur(6px);
    display:flex;align-items:center;justify-content:center;
    opacity:0;pointer-events:none;transition:opacity .25s ease;
  }
  .ajukan-success-overlay.is-visible{opacity:1;pointer-events:auto;}
  .ajukan-success-box{
    position:relative;display:flex;flex-direction:column;align-items:center;gap:6px;
    background:var(--white);border-radius:28px;padding:40px 40px 34px;
    box-shadow:0 32px 80px -18px rgba(31,157,124,.35),0 16px 40px -14px rgba(11,34,51,.4);
    text-align:center;width:min(90vw,380px);
    transform:scale(.8) translateY(14px);opacity:0;
    transition:transform .45s cubic-bezier(.34,1.56,.64,1),opacity .3s ease;
  }
  [data-theme="dark"] .ajukan-success-box{background:#0c1920;box-shadow:0 32px 80px -18px rgba(31,157,124,.22),0 16px 40px -14px rgba(0,0,0,.5);}
  .ajukan-success-overlay.is-visible .ajukan-success-box{transform:scale(1) translateY(0);opacity:1;}
  .ajukan-success-close{
  ...
  .ajukan-success-close:hover{background:#f1f4f5;color:#5b6b73;}
  [data-theme="dark"] .ajukan-success-close:hover{background:rgba(255,255,255,.08);color:#c3cdd2;}
  .ajukan-success-badge{position:relative;width:78px;height:78px;margin-bottom:6px;flex-shrink:0;}
  .ajukan-success-badge::before{
    content:"";position:absolute;inset:0;border-radius:50%;
    background:radial-gradient(circle,rgba(31,157,124,.25),transparent 70%);
    transform:scale(.3);opacity:0;
    animation:ajukan-success-glow .7s .05s cubic-bezier(.34,1.56,.64,1) forwards;
  }
  .ajukan-success-badge svg{position:relative;width:100%;height:100%;overflow:visible;}
  .ajukan-success-badge circle{
    fill:none;stroke:#1f9d7c;stroke-width:3.5;
    stroke-dasharray:214;stroke-dashoffset:214;
    transform:rotate(-90deg);transform-origin:39px 39px;
    animation:ajukan-success-ring .55s .05s cubic-bezier(.65,0,.35,1) forwards;
  }
  .ajukan-success-badge path{
    fill:none;stroke:#1f9d7c;stroke-width:4.5;stroke-linecap:round;stroke-linejoin:round;
    stroke-dasharray:52;stroke-dashoffset:52;
    animation:ajukan-success-check .3s .5s cubic-bezier(.65,0,.35,1) forwards;
  }
  @keyframes ajukan-success-glow{0%{transform:scale(.3);opacity:1;}100%{transform:scale(1.5);opacity:0;}}
  @keyframes ajukan-success-ring{to{stroke-dashoffset:0;}}
  @keyframes ajukan-success-check{to{stroke-dashoffset:0;}}
  .ajukan-success-heading{
    font-family:'Plus Jakarta Sans',system-ui,sans-serif;
    font-size:20px;font-weight:800;color:var(--navy);letter-spacing:-.01em;
    opacity:0;transform:translateY(6px);
    animation:ajukan-success-fade-up .4s .55s ease forwards;
  }
  [data-theme="dark"] .ajukan-success-heading{color:#eaf3f5;}
  .ajukan-success-text{
    margin-top:4px;font-size:13.5px;font-weight:500;color:#5b6b73;line-height:1.6;
    opacity:0;transform:translateY(6px);
    animation:ajukan-success-fade-up .4s .65s ease forwards;
  }
  [data-theme="dark"] .ajukan-success-text{color:#8a97a0;}
  .ajukan-success-text strong{color:var(--navy);}
  [data-theme="dark"] .ajukan-success-text strong{color:#eaf3f5;}
  .ajukan-success-email{
    margin-top:16px;display:inline-flex;align-items:center;gap:8px;
    padding:9px 16px;border-radius:999px;background:rgba(31,157,124,.1);color:#1f9d7c;
    font-size:12px;font-weight:700;
    opacity:0;transform:translateY(6px);
    animation:ajukan-success-fade-up .4s .72s ease forwards;
  }
  .ajukan-success-email svg{width:14px;height:14px;stroke:currentColor;fill:none;stroke-width:2;stroke-linecap:round;stroke-linejoin:round;flex-shrink:0;}
  @keyframes ajukan-success-fade-up{to{opacity:1;transform:translateY(0);}}
...
<?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(isset($submitted)): ?>
  <div class="ajukan-success-overlay" id="ajukanSuccessOverlay" aria-hidden="true">
    <div class="ajukan-success-box">
      ...
      <div class="ajukan-success-badge">
  ...
  <script>
    (function(){
      var overlay = document.getElementById('ajukanSuccessOverlay');
      if (!overlay) return;
      var closeBtn = document.getElementById('ajukanSuccessClose');
      ...
      // Elemen ini baru ada di DOM setelah applyLang() pertama kali jalan
      // (script terjemahan ada lebih atas), jadi panggil ulang supaya ikut
      // ke-translate kalau preferensi bahasanya EN.
